...
def iswav(a):
    if type(a)=='numpy.ndarray':
        ...
        a = np.array(a)
    return(a[['.wav' in file.lower() for file in a]])
...
data_files = ['\\HnKfile_list.dat','\\HnK_paths.dat','\\HnKdata.pickle','\\HnK_melspec.pickle','\\HnKsounds.pickle']
...
class data_format():
    ...
    def check_newbies(self):
        # print('ho')
        if all([os.path.exists(os.getcwd() + datafile) for datafile in data_files]):
            data_exists=True
            print('hey over here')
            self.print_to_label('Found data files')
            old_file_list = open(os.getcwd() + '\\HnKfile_list.dat','r')
            old_file_list = np.array([file[:-1] for file in old_file_list.readlines()])
            self.files = old_file_list
        else:
            data_exists=False
            old_file_list=np.array([])
            self.refresh(False,data_exists,old_file_list)
            return
        self.print_to_label('Scanning for new sounds...')
        # self.print_to_label('Scanning for new sounds...')
        for opendir in self.opendirs:
            names = iswav(os.listdir(opendir))
            if all([opendir + name in old_file_list for name in names]):
                pass
            else:
                self.print_to_label('New sounds found!')
                print('new sounds')
                self.refresh(False,data_exists,old_file_list) #new sounds found
                return
        self.print_to_label('Scan Complete.')
        self.refresh(True,data_exists,old_file_list)
        
    def refresh(self,fast,data_exists,old_file_list,PC=False):
        if fast == True:
            print(' 1 fast is True')
            if data_exists==True:
                print('data_exists is True')
            ...
            print(' 1 fast is False')
            # print('hey')
            names = np.concatenate([iswav(np.array([opendir+'\\'+name for name in os.listdir(opendir)])) for opendir in self.opendirs])
            # print(names)
            # self.print_to_label(names)
            if os.path.exists(os.getcwd()+'\\HnKfile_list.dat'):
                print(' 1 file list is exists')
                new_names = np.setdiff1d(names,old_file_list)
                new_file_list = np.concatenate([old_file_list,new_names])
            else:
                print(' 1 no file list!')
                new_names = names
                new_file_list = names
            # print("The new names are:", new_names)
            new_sounds = [hk.Aud(name) for name in new_names]
            '''data format:
            ...
            # self.print_to_label(self.new_data.shape)
            if data_exists==True:
                print(' 2 data_exists is true')
                # print('hey')
                with open(os.getcwd()+'\\HnKdata.pickle','rb') as f:
                    data = pickle.load(f)
                    f.close()
                ...
                with open(os.getcwd()+'\\HnK_melspec.pickle','rb') as g:
                    ...
                    g.close()
                all_specs = np.concatenate([old_melspec,new_specs])
                self.PCAaay(all_specs)
            else:
                print(' 2 data exists is false')
                self.data = new_data
                self.PCAaay(new_specs)
                self.sounds = [hk.Aud(file) for file in names]
            with open(os.getcwd()+'\\HnK_melspec.pickle','wb') as f:
                pickle.dump(self.melspecs,f,pickle.HIGHEST_PROTOCOL)
                f.close()
            with open(os.getcwd()+'\\HnKdata.pickle','wb') as f:
                pickle.dump(self.data,f,pickle.HIGHEST_PROTOCOL)
                f.close()
            with open(os.getcwd()+'\\HnKfile_list.dat','w') as f:
                for file in names:
                    f.write(file+'\n')
                f.close()
            with open(os.getcwd()+'\\HnKsounds.pickle','wb') as f:
            	# print('should be writing...')
            	pickle.dump(self.sounds,f,pickle.HIGHEST_PROTOCOL)
            	f.close()
            self.files=names
